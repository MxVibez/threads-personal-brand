  ...
  async createDemo(expertTelegramId: string): Promise<DraftView> {
    ...
    try {
      ...
      const draftResult = await client.query<{ id: string }>(
        ...
        [
          expertTelegramId,
          "Когда бизнесу нужен не ещё один сайт, а приложение внутри мессенджера",
          JSON.stringify([
            {
          ...
          JSON.stringify({
            ...
            risk: "Нужна проверка",
            audience: "Владельцы бизнеса и эксперты, которым нужно больше продаж из цифровых каналов",
            insight:
              "Материал объясняет ценность продукта через путь клиента и бизнес-результат, а не через список технологий.",
            evidence: [
              "Направление и услуги сформулированы Максимом.",
              "Цифры и неподтверждённые кейсы в тексте не используются.",
              "Перед реальной публикацией материал должен пройти одобрение."
            ]
          })
        ]
      );
      ...
      const segments = [
        "Если после рекламы клиент всё равно пишет менеджеру: «А что делать дальше?» — бизнесу часто нужен не ещё один сайт.",
        "Нужен короткий маршрут: человек открывает приложение прямо в Telegram, понимает предложение, отвечает на несколько вопросов и оставляет заявку без лишних переходов.",
        "Я проектирую Telegram Mini Apps, веб- и мобильные приложения для бизнеса и экспертов. Сначала собираю путь к продаже, потом уже выбираю экраны и технологии.",
        "Если хочешь понять, где приложение может убрать потери в твоей воронке, напиши «РАЗБОР»."
      ];
