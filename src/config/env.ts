...
const envSchema = z
  .object({
    ...
    THREADS_ANALYTICS_CACHE_SECONDS: z.coerce.number().int().min(60).max(86_400).default(900),
    THREADS_TOKEN_REFRESHED_AT: z.string().regex(/^$|^\d+$/).default(""),
    THREADS_TOKEN_EXPIRES_AT: z.string().regex(/^$|^\d+$/).default(""),
