...
printf 'Вставьте Threads App Secret (не будет виден): ' >&2
stty -echo
IFS= read -r threads_app_secret
stty echo
printf '\n' >&2
...
case "$threads_app_secret:$threads_access_token" in
  :*|*:) echo 'Секрет и токен не могут быть пустыми.' >&2; exit 1 ;;
esac
...
threads_response="$(curl --silent --show-error --fail \
  ...
  'https://graph.threads.net/v1.0/me')" || {
    unset threads_app_secret threads_access_token
...
if [ -z "$threads_user_id" ]; then
  unset threads_app_secret threads_access_token threads_response
...
upsert_env META_THREADS_APP_SECRET "$threads_app_secret"
