...
const TOPICS: Topic[] = [
  {
    ...
    title: "Как превратить рабочее наблюдение в полезную ветку",
    hook: "Полезная публикация начинается с одного точного наблюдения.",
    goal: "Проверить путь одобрения материала",
    discussionPotential: "Не оценивался",
    risk: "Требует проверки",
    audience: "[TARGET_AUDIENCE]",
    insight:
      "Нейтральная фикстура показывает структуру карточки. Она не основана на реальных исследованиях или метриках.",
    segments: [
      "Полезная публикация начинается с одного точного наблюдения.",
      "Затем автор объясняет, почему это важно выбранной аудитории.",
      "Перед публикацией нужно проверить факты, тон и возможные риски.",
      "Этот текст существует только для локальной проверки интерфейса."
    ],
    evidence: [
      "Демонстрационная запись не содержит рыночных данных.",
      "Факты и источники перед реальной публикацией не проверялись.",
      "Внешние интеграции выключены."
    ],
    sources: [
      { label: "[SOURCE_PLACEHOLDER]", meta: "источник не подключён" }
    ]
  },
  {
    id: "demo-question",
    ...
    mentions: 0,
    freshness: "без внешних данных",
    title: "Ответ на частый вопрос аудитории",
    hook: "Как объяснить сложную тему без лишних терминов?",
    goal: "Проверить отклонение и объяснение выбора",
    discussionPotential: "Не оценивался",
    risk: "Требует проверки",
    audience: "[TARGET_AUDIENCE]",
    insight:
      "Эта запись сохраняет рабочую структуру материала, но не содержит выводов о реальной аудитории.",
    segments: [
      "Сформулируйте вопрос так, как его задаёт ваша аудитория.",
      "Дайте короткое объяснение без недоказанных обещаний.",
      "Завершите понятным следующим действием."
    ],
    evidence: [
      "Нейтральная тестовая фикстура.",
      "Реальные сигналы не загружались.",
      "Формулировки требуют редакторской проверки."
    ],
    sources: [
      { label: "[SOURCE_PLACEHOLDER]", meta: "источник не подключён" }
    ]
  },
  {
    ...
    mentions: 0,
    freshness: "без внешних данных",
    title: "Короткое объяснение авторской позиции",
    hook: "Один тезис, один аргумент, один понятный вывод.",
    goal: "Проверить отображение одиночного поста",
    discussionPotential: "Не оценивался",
    risk: "Требует проверки",
    audience: "[TARGET_AUDIENCE]",
    insight:
      "Заглушка для будущей авторской позиции без привязки к теме или профессии.",
    segments: [
      "Один тезис, один аргумент, один понятный вывод. Замените эту заглушку материалом личного бренда после проверки источников."
    ],
    evidence: [
      "Нейтральная тестовая фикстура.",
      "Реальные источники отсутствуют."
    ],
    sources: [
      { label: "[SOURCE_PLACEHOLDER]", meta: "источник не подключён" }
    ]
  ...
  {
    id: "demo-light-format",
    version: 1,
    format: "Короткий пост",
    signal: "Демо",
    mentions: 0,
    freshness: "без внешних данных",
    title: "Лёгкое наблюдение между экспертными материалами",
    hook: "Иногда короткое наблюдение говорит о стиле автора больше длинной инструкции.",
    goal: "Проверить короткий формат",
    discussionPotential: "Не оценивался",
    risk: "Требует проверки",
    audience: "[TARGET_AUDIENCE]",
    insight:
      "Заглушка позволяет проверить смешанную очередь форматов без исходных клиентских текстов.",
    segments: [
      "Иногда короткое наблюдение говорит о стиле автора больше длинной инструкции."
    ],
    evidence: [
      "Нейтральная тестовая фикстура.",
      "Реальные реакции не измерялись."
    ],
    sources: [
      { label: "[SOURCE_PLACEHOLDER]", meta: "источник не подключён" }
    ]
    ...
    >
      ...
      <div className="post-author-row">
        <img
          ...
          alt="Аватар личного бренда — заглушка"
          width="38"
          height="38"
          decoding="async"
          fetchPriority="high"
...
const demoThreadsInsights: ThreadsInsightsDto = {
  ...
  themes: [
    { label: "[THEME_ONE]", averageViews: 0, posts: 0 },
    { label: "[THEME_TWO]", averageViews: 0, posts: 0 },
    { label: "[THEME_THREE]", averageViews: 0, posts: 0 }
  ]
};
